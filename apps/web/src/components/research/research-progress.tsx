    ...
    <section className="overflow-hidden rounded-xl border border-[var(--rule-strong)] bg-white shadow-[var(--shadow-soft)]">
      ...
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(15rem,0.8fr)_minmax(28rem,1.5fr)_auto] lg:items-center">
        <div className="flex items-center gap-4">
          <span
            className={`grid size-11 shrink-0 place-items-center rounded-md border ${isRetry ? "border-[#e8b5ae] bg-[#fff1ef] text-[var(--danger)]" : isReady ? "border-[#acd4bd] bg-[#edf8f1] text-[var(--success)]" : "border-[var(--accent-line)] bg-[var(--accent-soft)] text-[var(--accent)]"}`}
          >
            {isRetry ? (
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-2">
              <h2 className="text-lg font-extrabold tracking-[-0.03em] text-[var(--ink)]">
                Field collection
              </h2>
              <span
                className={`status-pill ${isRetry ? "bg-[#fff1ef] text-[var(--danger)]" : isReady ? "bg-[#edf8f1] text-[var(--success)]" : "bg-[var(--accent-soft)] text-[var(--accent)]"}`}
              >
                {status.replaceAll("_", " ")}
              </span>
            </div>
        ...
        <div>
          {!progress ? (
            ...
            <div
              className="grid grid-cols-3 divide-x divide-[var(--rule)]"
              aria-live="polite"
            >
              <ProgressMetric
                label="Discovered"
          ...
          {isRunning || isActive ? (
            <>
              <LoaderCircle
                ...
                size={16}
              />
              Collecting {progress?.progress ?? 0}%
            </>
          ) : isRetry ? (
            <>
      ...
      {error || progress?.error ? (
        <div
          className="border-t border-[#e8b5ae] bg-[#fff1ef] px-5 py-3 text-sm font-semibold text-[var(--danger)]"
          role="alert"
        >
          {error ?? progress?.error}
        </div>
        ...
        <div className="flex flex-col gap-1 border-t border-[var(--rule)] bg-[var(--paper-subtle)] px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-[var(--ink-faint)]">
            Records are normalized and deduplicated before analysis.
          </p>
          <p
            className="max-w-xs truncate font-mono text-[0.62rem] text-[var(--ink-faint)]"
            title={researchId}
          >
            ID / {researchId}
          </p>
...
function ProgressMetric({
  ...
  return (
    <div className="px-4 first:pl-0 last:pr-0">
      <dt className="font-mono text-[0.62rem] font-semibold tracking-[0.08em] text-[var(--ink-faint)] uppercase">
        {label}
      </dt>
      <dd
        className={`mt-1 text-xl font-extrabold tabular-nums ${danger ? "text-[var(--danger)]" : "text-[var(--ink)]"}`}
      >
        {value}
      </dd>
    </div>
