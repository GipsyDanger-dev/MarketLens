    ...
    <section className="overflow-hidden rounded-xl border border-[var(--rule-strong)] bg-white shadow-[var(--shadow-soft)]">
      <header className="grid gap-5 border-b border-[var(--rule)] p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-extrabold tracking-[-0.045em] text-[var(--ink)]">
            Explainable competitor ranking
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--ink-soft)]">
            Compare market presence through rating strength, review authority,
            local density, and proximity—not a black-box verdict.
          </p>
        </div>
        <div className="flex items-center gap-3 rounded-md border border-[var(--accent-line)] bg-[var(--accent-soft)] px-4 py-3">
          ...
          />
          <div>
            <p className="font-mono text-[0.58rem] tracking-[0.09em] text-[var(--ink-faint)] uppercase">
              Field size
            </p>
            <p className="mt-0.5 text-sm font-extrabold text-[var(--ink)]">
              {ranked.length} ranked places
            </p>
          </div>
        </div>
      </header>
            ...
            {ranked.map((place, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      className={`font-mono text-[0.66rem] ${isSelected ? "text-[#8fa8ff]" : "text-[var(--ink-faint)]"}`}
                    >
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <span className="min-w-0">
                      <span
                        className={`block truncate text-sm font-bold ${isSelected ? "text-white" : "text-[var(--ink)]"}`}
                      >
                        {place.name}
                      </span>
                      <span
                        className={`mt-1 block text-xs ${isSelected ? "text-[#9fadc4]" : "text-[var(--ink-faint)]"}`}
                      >
                        {place.rating ?? "—"} rating ·{" "}
                    ...
                    <span
                      className={`font-mono text-sm font-extrabold tabular-nums ${isSelected ? "text-[#8fa8ff]" : "text-[var(--accent)]"}`}
                    >
                      {placeScore}
                    </span>
                  </button>
        ...
        <article className="p-5 sm:p-7 lg:p-8">
          <div className="grid gap-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
            <div>
              ...
              <h3 className="type-display mt-3 text-[clamp(2.7rem,6vw,5rem)] leading-[0.9] tracking-[-0.055em] text-[var(--ink)]">
                {selected.name}
              </h3>
              <p className="mt-5 max-w-2xl text-sm leading-6 text-[var(--ink-soft)]">
                {score?.explanation ??
                  "No score explanation is available for this place."}
              </p>
            </div>
            <div className="grid size-28 shrink-0 place-items-center rounded-full border border-[var(--accent-line)] bg-[var(--accent-soft)] text-center shadow-[inset_0_0_0_8px_white]">
              <span>
                <strong className="type-display block text-4xl leading-none text-[var(--accent)]">
                  {selectedScore}
                </strong>
                <span className="mt-1 block font-mono text-[0.55rem] font-bold tracking-[0.08em] text-[var(--ink-faint)] uppercase">
                  Overall
                </span>
              </span>
            </div>
          ...
          <div className="mt-8 border-y border-[var(--rule)] py-2">
            {components.length === 0 ? (
              <p className="py-3 text-sm text-[var(--ink-faint)]">
                No component scores available.
              </p>
            ) : null}
            {components.map(([name, value]) => (
              <div
                className="grid grid-cols-[7rem_minmax(0,1fr)_2.7rem] items-center gap-3 py-2.5"
                key={name}
              >
                <span className="truncate text-xs font-semibold capitalize text-[var(--ink-soft)]">
                  {name.replaceAll("_", " ")}
                </span>
                ...
                <span className="text-right font-mono text-[0.66rem] font-bold text-[var(--ink)]">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <NarrativeBlock
              ...
              value={narrative.strengths.join(", ") || "No dominant signal"}
            />
            <NarrativeBlock
              icon={<Gauge aria-hidden="true" size={17} />}
              label="Relative weaknesses"
              value={narrative.weaknesses.join(", ") || "No material weakness"}
            />
          </div>
          ...
          <div className="mt-6 rounded-lg border border-[var(--rule)] bg-[var(--paper-subtle)] p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-[var(--ink-soft)]">
              {nearby
                ...
                .join(" · ") ||
                "No other ranked place is within one kilometre."}
            </p>
          </div>
        </article>
      </div>
      ...
      <div className="border-t border-[var(--rule-strong)]">
        <div className="flex items-center justify-between px-5 py-4 sm:px-7">
          <div>
            ...
            <h3 className="mt-1 text-lg font-extrabold tracking-[-0.03em] text-[var(--ink)]">
              Comparison register
            </h3>
          </div>
          <ArrowRight
        ...
        <div className="grid gap-px bg-[var(--rule)] md:hidden">
          {ranked.map((place, index) => (
            <div
              className="grid grid-cols-[2rem_minmax(0,1fr)_auto] items-center gap-3 bg-white px-5 py-4"
              key={place.id}
            >
              <span className="font-mono text-[0.62rem] text-[var(--ink-faint)]">
                {String(index + 1).padStart(2, "0")}
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 block text-xs text-[var(--ink-faint)]">
                  {place.rating ?? "—"} rating · {place.reviewCount ?? "—"}{" "}
                  reviews
                </span>
              </span>
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[620px] text-left text-sm">
            <thead className="bg-[var(--paper-subtle)] font-mono text-[0.62rem] tracking-[0.08em] text-[var(--ink-faint)] uppercase">
              <tr>
                <th className="px-7 py-3">Rank / competitor</th>
                <th className="px-4 py-3">Rating</th>
                <th className="px-4 py-3">Reviews</th>
                <th className="px-7 py-3 text-right">Score</th>
              </tr>
            </thead>
            <tbody>
              {ranked.map((place, index) => (
                <tr
                  className="border-t border-[var(--rule)] text-[var(--ink-soft)]"
                  key={place.id}
                >
                  <td className="px-7 py-3.5">
                    <span className="mr-4 font-mono text-[0.65rem] text-[var(--ink-faint)]">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <strong className="text-[var(--ink)]">{place.name}</strong>
                  </td>
                  ...
                  <td className="px-7 py-3.5 text-right font-mono font-bold text-[var(--accent)]">
                    {Math.round(
                      (place.competitorScores[0]?.overallScore ?? 0) * 100,
                    )}
                    %
