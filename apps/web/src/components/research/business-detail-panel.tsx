...
export function BusinessDetailPanel({
    ...
    return (
      <section className="flex min-h-80 flex-col justify-between rounded-xl border border-dashed border-[var(--rule-strong)] bg-[var(--paper-subtle)] p-6">
        ...
        <div>
          ...
          <h2 className="type-display mt-3 text-4xl leading-[0.95] tracking-[-0.05em] text-[var(--ink)]">
            Choose a point on the map.
          </h2>
          <p className="mt-4 text-sm leading-6 text-[var(--ink-soft)]">
            Select a marker to inspect the address, phone, website, email,
            social profiles, and original source record.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <section
      className="overflow-hidden rounded-xl border border-[var(--rule-strong)] bg-white shadow-[var(--shadow-soft)]"
      id={`business-${place.id}`}
    >
      <div className="bg-[var(--graphite)] p-6 text-white">
        <div className="flex items-center justify-between gap-3">
          <p className="font-mono text-[0.64rem] font-bold tracking-[0.12em] text-[#93a9da] uppercase">
            Selected business
          </p>
          <span className="size-2 rounded-full bg-[#6f8fff] shadow-[0_0_0_4px_rgb(49_94_245/0.18)]" />
        </div>
        <h2 className="type-display mt-6 text-4xl leading-[0.92] tracking-[-0.05em] text-white">
          {place.name}
        </h2>
        <p className="mt-3 text-sm text-[#afbdd2]">
          {place.category ?? "Uncategorized business"}
        </p>
      </div>
      ...
      <dl className="divide-y divide-[var(--rule)]">
        ...
        <Detail
          ...
          label="Website"
          value={website ? new URL(website).hostname.replace("www.", "") : null}
        />
        <Detail
          href={sourceUrl}
          icon={<Database aria-hidden="true" size={16} />}
          label="Source"
          value={sourceUrl ? "Open original record" : null}
        />
      </dl>
        ...
        {socialLinks.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {socialLinks.map(([network, href]) => (
              <a
                className="inline-flex min-h-10 items-center gap-2 rounded-md border border-[var(--rule-strong)] bg-white px-3 text-xs font-bold capitalize text-[var(--ink)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
                href={href}
                key={network}
                rel="noreferrer"
                target="_blank"
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--ink-faint)]">
            No social profile was supplied by this provider.
          </p>
        )}
      </div>
    </section>
...
function Detail({
  ...
  return (
    <div className="grid grid-cols-[2rem_minmax(0,1fr)] gap-2 px-5 py-4">
      ...
      <div>
        <dt className="font-mono text-[0.59rem] font-bold tracking-[0.09em] text-[var(--ink-faint)] uppercase">
          {label}
        </dt>
        <dd className="mt-1 break-words text-sm leading-5 text-[var(--ink-soft)]">
          {value ? (
            href ? (
              <a
                className="text-link inline-flex items-start gap-1.5 font-semibold"
                href={href}
