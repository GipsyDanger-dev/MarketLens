        ...
        <header className="dark-grid flex flex-col justify-between bg-[var(--graphite)] p-6 text-white sm:p-8">
          <div>
            ...
            <p className="mt-5 max-w-lg text-sm leading-6 text-[#b5c1d4]">
              Every file is generated from the same persisted study, keeping the
              record consistent across analysis and presentation.
            </p>
          </div>
        ...
        <div>
          <div className="border-b border-[var(--rule)] p-5 sm:p-7">
            ...
            <h3 className="mt-2 text-2xl font-extrabold tracking-[-0.04em] text-[var(--ink)]">
              Choose the file for the next job
            </h3>
            <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--ink-soft)]">
              Downloads are created on demand and include the latest persisted
              research snapshot.
            </p>
          </div>
          ...
          <div className="divide-y divide-[var(--rule)]">
            {FORMATS.map(({ description, extension, icon: Icon, label }) => (
              <div
                className="grid gap-4 p-5 sm:grid-cols-[3rem_minmax(0,1fr)_auto] sm:items-center sm:px-7"
                key={extension}
              >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-sm bg-[var(--paper-muted)] px-1.5 py-0.5 font-mono text-[0.58rem] font-bold tracking-[0.06em] text-[var(--ink-faint)] uppercase">
                      .{extension}
                    </span>
                  </div>
          ...
          <div className="flex items-start gap-3 border-t border-[var(--rule)] bg-[var(--paper-subtle)] px-5 py-4 text-xs leading-5 text-[var(--ink-faint)] sm:px-7">
            <PackageCheck
              ...
              size={16}
            />
            Provider attribution and source metadata remain attached where the
            chosen format supports them.
          </div>
        </div>
      </div>
    </section>
