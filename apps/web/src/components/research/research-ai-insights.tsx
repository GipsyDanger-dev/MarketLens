      ...
      <div className="grid lg:grid-cols-[minmax(20rem,0.62fr)_minmax(0,1.38fr)]">
        <header className="dark-grid flex flex-col justify-between bg-[var(--graphite)] p-6 text-white sm:p-8">
          <div>
            ...
            <p className="mt-8 font-mono text-[0.65rem] font-bold tracking-[0.13em] text-[#93a9da] uppercase">
              Optional interpretation
            </p>
            <h2 className="type-display mt-4 text-[clamp(3rem,6vw,5rem)] leading-[0.9] tracking-[-0.055em] text-white">
              Evidence, interpreted with boundaries.
            </h2>
            <p className="mt-5 max-w-lg text-sm leading-6 text-[#b5c1d4]">
              AI receives this research dataset and its computed metrics—not an
              open-ended prompt. Every output remains an interpretation to
              validate.
            </p>
          </div>
          ...
          <div className="mt-10 border-t border-white/14 pt-5">
            <div className="flex items-start gap-3">
              <ShieldCheck
                aria-hidden="true"
                className="mt-0.5 shrink-0 text-[#d58850]"
                size={18}
              />
              <p className="text-xs leading-5 text-[#9fadc4]">
                Facts, interpretation, and limitations are kept visibly separate
                in the generated memo.
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="p-5 sm:p-8">
          ...
          {error ? (
            <p
              className="mt-5 rounded-md border border-[#dfc39d] bg-[#fff8ec] px-4 py-3 text-sm font-semibold text-[#78471e]"
              role="status"
            >
              {error}
            </p>
          ...
          {snapshot ? (
            ...
            <div className="grid min-h-72 place-items-center py-10 text-center">
              <div className="max-w-sm">
                <span className="mx-auto grid size-11 place-items-center rounded-full border border-[var(--rule)] text-[var(--ink-faint)]">
                  <Sparkles aria-hidden="true" size={18} />
                </span>
                <p className="mt-4 text-sm font-bold text-[var(--ink)]">
                  The evidence remains primary.
                </p>
                <p className="mt-2 text-xs leading-5 text-[var(--ink-faint)]">
                  Generate a memo only when an AI provider has been configured
  ...
  return (
    <div className="divide-y divide-[var(--rule)]">
      {sections.map(([title, items], index) => (
        <section
          className="grid gap-3 py-5 sm:grid-cols-[2rem_10rem_minmax(0,1fr)]"
          key={title}
        >
          <span className="font-mono text-[0.62rem] font-bold text-[var(--accent)]">
          ...
          {items.length ? (
            <ul className="space-y-2 text-sm leading-6 text-[var(--ink-soft)]">
              {items.map((item) => (
                <li
                  className="border-l-2 border-[var(--accent-line)] pl-3"
                  key={item}
                >
                  {item}
                </li>
